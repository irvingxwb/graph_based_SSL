...
# generate instance with given text and label
# differ from read_instance
def gene_instance(input_text, input_label, word_alphabet, char_alphabet, feature_alphabets, label_alphabet, number_normalized,
                  max_sent_length, split_token='\t', char_padding_size=-1, char_padding_symbol='</pad>'):
    feature_num = len(feature_alphabets)
    instence_texts = []
    instence_Ids = []
    words = []
    ...
    if input_label is None:
        for sent in input_text:
            for word in sent:
                ...
                if number_normalized:
                    word = normalize_word(word)
                labels.append(label_alphabet.UNKNOWN)
                word_Ids.append(word_alphabet.get_index(word))
                label_Ids.append(label_alphabet.default_index)
                ## get features - not used, just remain for further development
                feat_list = []
                feat_Id = []
                for idx in range(feature_num):
                    # hack code
        ...
        assert len(input_text) == len(input_label)
        for sent, label_sent in zip(input_text, input_label):
            assert len(sent) == len(label_sent)
            for word, label in zip(sent, label_sent):
                words.append(word)
                if number_normalized:
                    word = normalize_word(word)
                labels.append(label)
