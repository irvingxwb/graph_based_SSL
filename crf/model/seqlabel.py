...
class SeqLabel(nn.Module):
    def __init__(self, data):
        ...
        logger.info("char feature extractor: " + data.char_feature_extractor)
        logger.info("word feature extractor: " + data.word_feature_extractor)
        logger.info("use_char: " + str(data.use_char))
        logger.info("use crf:" + str(self.use_crf))
    ...
    def decode_nbest(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths,
        ...
        if not self.use_crf:
            logger.info("Nbest output is currently supported only for CRF! Exit...")
            exit(0)
        outs = self.word_hidden(word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths,
                                char_seq_recover)
        batch_size = word_inputs.size(0)
