...
class CRF(nn.Module):
    ...
    def __init__(self, label_size, gpu):
        ...
        init_transitions = torch.zeros(self.label_size + 2, self.label_size + 2)
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        assert (tag_size == self.label_size + 2)
        ...
        for idx, cur_values in seq_iter:
            ...
            # partition = utils.switch(partition, cur_partition, mask[idx].view(bat_size, 1).expand(bat_size, self.label_size)).view(bat_size, -1)
    ...
    def _viterbi_decode_probs(self, feats, mask):
        """
            input:
                feats: (batch, seq_len, self.tag_size+2)
                mask: (batch, seq_len)
